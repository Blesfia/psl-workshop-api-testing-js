...
describe('Given a POST PATCH tests', () => {
  ...
  it('Then the user logged must have at least 1 public repo', () => agent.get('https://api.github.com/user')
  ...
  describe(`When i get the ${repoToEdit} repository`, () => {
    ...
    describe('And the issue will be create', () => {
      ...
      it('Then the issue must allow the edit action and work correctly', () => agent.patch(`https://api.github.com/repos/${user.login}/${repoToEdit}/issues/${issue.number}`, { body: 'Body Issue' })
        .auth('token', process.env.ACCESS_TOKEN).then((response) => {
